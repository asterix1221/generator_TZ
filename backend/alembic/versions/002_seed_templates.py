"""Seed templates table with all project types and complexity levels
...
TEMPLATE_DATA = {
    ...
    "Mobile": {
        1: {
            ...
            "db_requirements": ["1 таблица", "Локальное хранилище"],
        ...
        2: {
            ...
            "db_requirements": ["Реляционная БД", "Синхронизация данных"],
        ...
        3: {
            ...
            "db_requirements": ["PostgreSQL + Redis", "Облачное хранилище"],
        ...
        4: {
            ...
            "db_requirements": ["Распределенная БД", "Шифрование"],
    ...
    "Other": {
        1: {
            ...
            "functional_requirements": ["Базовый функционал по выбору"],
        ...
        2: {
            ...
            "functional_requirements": ["Расширенный функционал", "База данных"],
            "tech_stack": ["По выбору студента"],
            "features": ["CRUD"]
        },
        3: {
            ...
            "functional_requirements": ["Полноценный функционал", "Внешние интеграции"],
            "tech_stack": ["По выбору студента"],
            "features": ["API"]
        },
        4: {
...
def upgrade() -> None:
    ...
    for project_type, complexities in TEMPLATE_DATA.items():
        for complexity, structure in complexities.items():
            structure_json = json.dumps(structure, ensure_ascii=False)
            conn.execute(
                text("""
                    INSERT INTO templates (type, complexity, structure)
                    VALUES (:type, :complexity, CAST(:structure AS jsonb))
                    ON CONFLICT (type, complexity) DO NOTHING
                """),
                {
                    "type": project_type,
...
def downgrade() -> None:
    conn = op.get_bind()
    for project_type in TEMPLATE_DATA.keys():
        conn.execute(
            text("DELETE FROM templates WHERE type = :type"),
            {"type": project_type}
        )